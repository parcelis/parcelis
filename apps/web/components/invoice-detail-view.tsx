...
function InvoiceDetailContainer({ children, embedded }: { children: ReactNode; embedded: boolean }) {
  if (embedded) {
    return <section className="bg-parcelis-porcelain dark:bg-parcelis-slate">{children}</section>;
  }

  return <main className="min-h-screen bg-parcelis-porcelain dark:bg-parcelis-slate">{children}</main>;
}
