...
`Dockerfile.app` is the shared application Dockerfile. Its `app` target builds `apps/web` and `apps/api` into the modular `app` image, and its `all-in-one` target additionally bundles the documentation site into the single `all` application image. Nginx routes browser requests to Next.js and forwards `/trpc/*` and `/api/*` to NestJS inside the container. The production Compose proxy is the `proxy` image and routes `/docs/*` to the documentation container. A release workflow publishes `apps`, `docs`, `all`, and `proxy` to Docker Hub when a GitHub release is published.
